...
Figures 1, 3, and 4 are rendered from the locked results and included
below (sources in \texttt{paper/figures/}). Figures 2 and 5 are
specified for the camera-ready version; their underlying numbers are
already reported in Section 7.
...
\textbf{Figure 2 (single-stage sensitivity/PPV frontier; to render).}
DS2 VEB PPV as a function of sensitivity for single-stage models, one
curve per seed, with the three named operating points marked.
\emph{Message: a single operating point cannot reach both sensitivity
\textgreater= 0.90 and PPV \textgreater= 0.60 reliably across seeds.}

\textbf{Figure 5 (SVEB negative result; to render).} DS2 SVEB
sensitivity versus PPV per seed for the SVEB specialist, contrasted with
the same architecture's SVEB sensitivity on 12-lead INCART.
\emph{Message: single-lead SVEB is unstable and low-precision; the same
